...
// If the estimated time already passed, don't add another day
                    ...
                    <div x-data="{ expired: false, countdown: '{{ $estimatedDateTime }}' }" x-init="let timer = setInterval(() => {
                        ...
                        <template x-if="!expired">
                            <x-countdown :expires="$estimatedDateTime" class="flex space-x-2">
                                <div class="p-2 px-3 border rounded-xl"
                                    :class="{ 'text-red-600 border-red-600': expired }">
                                ...
                                <div class="p-2 px-3 border rounded-xl"
                                    :class="{ 'text-red-600 border-red-600': expired }">
                                    <h1 class="text-xl font-bold" x-text="timer.minutes">{{ $component->minutes() }}
                                    </h1>
                                    <h1 class="text-xs leading-3">min</h1>
                                </div>
                                <div class="p-2 px-3 border rounded-xl"
                                    :class="{ 'text-red-600 border-red-600': expired }">
                                    <h1 class="text-xl font-bold" x-text="timer.seconds">{{ $component->seconds() }}
                                    </h1>
...
<div>
    ...
    @else
            ...
            <ol class="relative border-s border-yellow-500 ">
                ...
                @if ($order->is_arrived)
                    <li class="mb-3 ms-4">
                        ...
                        <p class="mb-4 text-xs font-normal text-gray-500 dark:text-gray-400">Your driver is here!
                            Please
        ...
        <div class="mt-10 flex justify-center">
            @if ($order->is_arrived)
                <x-button href="{{ route('customer.transaction') }}" label="PROCEED" positive
                    right-icon="arrow-right" rounded class="font-semibold px-6" />
            @endif
        </div>
    @endif
</div>
